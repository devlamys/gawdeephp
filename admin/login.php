    ...
    <main class="auth-shell">
        <section class="auth-visual">
            ...
            <div class="auth-visual__copy">
                ...
                <p>Manage the storefront, orders, payments, courier workflow and AI publishing from one calm workspace.
                </p>
            </div>
        ...
        <section class="auth-panel">
            <div class="auth-card">
                ...
                <p><?= $isSetup ? 'No default credentials are used. Create the first secure administrator now.' : 'Use your administrator account to continue.' ?>
                </p>
                <?php if ($error): ?>
                    <div class="admin-alert admin-alert--error"><i
                            class="ph ph-warning-circle"></i><?= htmlspecialchars($error) ?></div><?php endif; ?>
                <form method="post" class="admin-form auth-form">
                    ...
                    <?php if ($isSetup): ?>
                        <label><span>Full name</span>
                            <div class="field-icon"><i class="ph ph-user"></i><input name="name" autocomplete="name"
                                    required value="<?= htmlspecialchars((string) ($_POST['name'] ?? '')) ?>"
                                    placeholder="Store administrator"></div>
                        </label>
                    <?php endif; ?>
                    <label><span>Email address</span>
                        <div class="field-icon"><i class="ph ph-envelope-simple"></i><input type="email" name="email"
                                autocomplete="email" required
                                value="<?= htmlspecialchars((string) ($_POST['email'] ?? '')) ?>"
                                placeholder="you@gawdee.com"></div>
                    </label>
                    <label><span>Password</span>
                        <div class="field-icon"><i class="ph ph-lock-key"></i><input type="password" name="password"
                                autocomplete="<?= $isSetup ? 'new-password' : 'current-password' ?>" required
                                minlength="<?= $isSetup ? '12' : '1' ?>"
                                placeholder="<?= $isSetup ? 'Minimum 12 characters' : 'Your password' ?>"></div>
                    </label>
                    <?php if ($isSetup): ?>
                        <label><span>Confirm password</span>
                            <div class="field-icon"><i class="ph ph-shield-check"></i><input type="password"
                                    name="password_confirm" autocomplete="new-password" required minlength="12"
                                    placeholder="Repeat the password"></div>
                        </label>
                    <?php endif; ?>
                    <button class="admin-button admin-button--primary admin-button--full"
                        type="submit"><?= $isSetup ? 'Create admin & continue' : 'Sign in' ?> <i
