...
<head>
    ...
    <link rel="stylesheet" href="assets/css/gawdee-reference.css">
    <style>:root{--site-body-font:<?= $siteBodyFont ?>;--site-heading-font:<?= $siteHeadingFont ?>;--site-base-font-size:<?= $siteBaseFontSize ?>px}</style>
    <script>document.documentElement.classList.add('js');</script>
</head>
<body class="<?= htmlspecialchars($bodyClass) ?>">
<a class="skip-link" href="#main-content">Skip to content</a>

<div class="promo-strip reference-promo">
    <div class="container reference-promo__grid">
        <span><i class="ph ph-seal-check"></i> Pure. Natural. Trusted.</span>
        <span><i class="ph ph-cow"></i> 100% A2 Ghee</span>
        <span><i class="ph ph-flask"></i> No Preservatives</span>
        <span><i class="ph ph-plant"></i> Handcrafted on Indian Farms</span>
        <span><i class="ph ph-recycle"></i> Plastic Neutral</span>
        <span><i class="ph ph-truck"></i> Free Shipping on Orders ₹<?= number_format((int) gawdee_setting('free_shipping_threshold', '999')) ?>+</span>
    </div>
</div>
...
<header class="commerce-header" data-header>
    <div class="container commerce-header__main">
        ...
        <a class="commerce-logo" href="index.php" aria-label="Gawdee home">
            <img src="assets/images/logo.png" alt="Gawdee — The Mother of Organic Nutrition">
        </a>
        ...
        <div class="commerce-actions">
            <a class="commerce-action desktop-only" href="<?= $headerCustomer ? 'account.php' : 'login.php' ?>">
                <i class="ph <?= $headerCustomer ? 'ph-user-circle-check' : 'ph-user' ?>" aria-hidden="true"></i><span><?= $headerCustomer ? htmlspecialchars(explode(' ', trim((string) $headerCustomer['name']))[0]) : 'Login' ?></span>
            </a>
    ...
    <div class="container commerce-nav-shell reference-nav-shell">
        <nav class="commerce-nav" aria-label="Product categories">
            <a class="<?= $isCommerceHome ? 'is-active' : '' ?>" href="index.php">Home</a>
            <a href="products.php?category=ghee" data-nav-filter="ghee">A2 Ghee <i class="ph ph-caret-down"></i></a>
            <a href="products.php?category=honey" data-nav-filter="honey">Natural Honey <i class="ph ph-caret-down"></i></a>
            <a href="products.php?category=sugar" data-nav-filter="sugar">Organic Foods <i class="ph ph-caret-down"></i></a>
            <a href="products.php?category=wellness" data-nav-filter="wellness">Wellness <i class="ph ph-caret-down"></i></a>
            <a href="index.php#farms">Our Farms</a>
